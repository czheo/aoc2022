...
fn decrypt(input: &mut Vec<(usize, i64)>, index: &mut Vec<usize>) {
    let l = input.len();
    let mut i = 0;
    while i < l {
        let j = index[i];
 ...
 *   8 7 6
 */
fn next_position(n: i64, j: usize, l: usize)-> usize {
    if n == 0 {
        j
    } else if n > 0 {
...
fn shift(j: usize, p: usize, input: &mut Vec<(usize, i64)>, index: &mut Vec<usize>) {
    let (x, m) = input[j];
    let mut j = j;
    while j != p {
        let (y, n) = if j < p {
...
fn parse() -> Vec<(usize, i64)> {
    std::io::stdin().lines()
        .map(|x| x.unwrap().parse().unwrap())
        .map(|x:i64| x * 811589153)
        .enumerate()
        .collect()
}
