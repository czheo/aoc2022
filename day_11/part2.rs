...
enum Pred {
    Pred(u64, usize, usize)
}

fn main() {
    let (mut items, ops, tests) = parse();
    let mut cnts = vec![0; items.len()];
    // use modulo of the product of all dividors to limit range
    let base: u64 = tests.iter().map(|Pred::Pred(n, _, _)| n).product();
    for round in 1..=10000 {
        ...
        if round == 1 || round == 20 || round % 1000 == 0 {
            println!("round {}: {:?} {:?}", round, cnts, items);
        }
    }
    println!("{}", worry_level(&cnts));
}

fn worry_level(cnts: &Vec<u64>) -> u64 {
    let mut m1 = 0;
    let mut m2 = 0;
    for c in cnts {
        if *c > m1 {
...
fn eval_pred(pred: &Pred, x: u64) -> usize {
...
fn parse_list(s: &str) -> VecDeque<u64> {
