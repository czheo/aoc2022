...
fn main() {
    ...
    let ans = dfs(start, 1<<start, 30, &flows, &graph, &mut cache);
    println!("{}", ans);
    println!("stats searched = {}", cache.len());
}

fn dfs(node: usize, visited: u64, time: u64,
    flows: &Vec<u64>, graph: &Vec<Vec<usize>>,
    cache: &mut HashMap<(usize, u64, u64), u64>) -> u64 {
    if time <= 0 {
        return 0;
    }
    let key = (node, visited, time);
    ...
    if flows[node] > 0 && !is_visisted(node, visited) {
        ...
        ret = max(ret, flows[node] * (time - 1) + dfs(node, v, time - 1, flows, graph, cache));
    }
    for nb in graph[node].iter() {
        ret = max(ret, dfs(*nb, visited, time - 1, flows, graph, cache));
    }
